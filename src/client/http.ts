  ...
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    const req = driver.request(
      ...
      (res) => {
        ...
        res.on("error", (err) => {
          ...
          reject(new SmardNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new SmardNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      // A timeout destroy already passes an SmardNetworkError; don't double-wrap.
      reject(err instanceof SmardNetworkError ? err : new SmardNetworkError(err.message, { cause: err }));
    });
