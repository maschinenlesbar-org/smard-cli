...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    try {
      ...
      reject(new SmardNetworkError(`Invalid URL: ${request.url}`));
      return;
    }
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new SmardNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${request.url}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            fail(new SmardResponseTooLargeError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          fail(new SmardNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      ...
      timer = setTimeout(() => {
        const err = new SmardNetworkError(`Request timed out after ${timeoutMs}ms`);
        fail(err);
        req.destroy(err);
    ...
    req.on("error", (err) => {
      // A timeout destroy already passes an SmardNetworkError; don't double-wrap.
      fail(err instanceof SmardNetworkError ? err : new SmardNetworkError(err.message, { cause: err }));
    });
