...
test("performs a real GET and returns status, headers and body", async () => {
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({ method: "GET", url: `${baseUrl}/app/chart_data/` });
      assert.equal(resp.status, 200);
      assert.equal(resp.headers["content-type"], "application/json");
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), { path: "/app/chart_data/" });
    },
  );
});

test("rejects an unsupported protocol with SmardNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof SmardNetworkError && /timed out after 300ms/.test(err.message),
      );
